...
class VerifyMobileController extends Controller
{
    public function __invoke(Request $request)
    {
       // dd($request);
        ...
        if ($request->user()->hasVerifiedMobile()) return redirect()->to(RouteServiceProvider::DASHBOARD);
        ...
        if ($request->code === auth()->user()->mobile_verify_code) {
            ...
            if ($secondsOfValidation > 0 &&  $request->user()->mobile_verify_code_sent_at->diffInSeconds() > $secondsOfValidation) {
                ...
                return redirect()->to(RouteServiceProvider::DASHBOARD)->with(['message' => __('mobile.verified')]);
            }
        }
